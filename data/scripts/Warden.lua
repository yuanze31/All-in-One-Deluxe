...
function menu()
    ...
    if World.CheatsEnabled then
        if World.ImmediateObjects then
            ...
            extend(miz, {{'need_ImmediateObjects'}})
        end
    end
...
function updateTooltipP(p)
    ...
    if p.last_extortion then
        ...
        if ago < 60 then
            p.Tooltip = {'fc__tt__p', p.extortion_count, 'X', p.fight_count, 'F', p.money_taken, 'M', ago, 'L',
                         p.BoilingPoint_lowest, "A", BoilingPoint_current, "B"}
            return
        end
    end
    ...
    if ttt == 'sick' then
        ...
        if p.sickness_last_date then
            p.Tooltip = {'ptt/sick/damaged', round(p.sickness_points), 'P', round(p.sickness_points_exercised), 'X',
                         round(p.sickness_points_food), 'F', round(p.sickness_points_damaged), 'L',
                         math.floor(p.sickness_last_damage * 100 + 0.5), 'D', (today - p.sickness_last_date), 'A'}
        else
            p.Tooltip = {'ptt/sick/init', round(p.sickness_points), 'P', round(p.sickness_points_exercised), 'X',
                         round(p.sickness_points_food), 'F', round(p.sickness_points_damaged), 'L'}
        end
    ...
    elseif ttt == 'rc' then
        ...
        elseif p.bonus_count > 0 then
            p.Tooltip = {"ptt/rc/bonus", p.eval_chance_first, "F", p.eval_chance_lowest, "L", p.eval_days, "D",
                         p.eval_chance_improved, "I", p.bonus_count, "B"}
        else
            p.Tooltip = {"ptt/rc/no_bonus", p.eval_chance_first, "F", p.eval_chance_lowest, "L", p.eval_days, "D",
                         p.eval_chance_improved, "I"}
        end
