...
function updateTT()
	...
	elseif this.this_is_medicalward and World.TimeIndex - this.last_doc_was_here > 2 then
		this.Tooltip = {'medcab__tt__doc_away_time', this.treat_count, 'X', this.treat_cost, 'Y', math.floor(World.TimeIndex - this.last_doc_was_here), 'M'}
	else
...
function menu()
	...
	local miz = {
		mi_level,
		{"call_doctor_for_injured", "Button", tostring( this.call_doctor_for_injured ), "B"},
...
function additionalAidAround()
	...
	local h = look_around({'Prisoner', "Guard", "DogHandler", "Workman", "ArmedGuard", "Cook", "Janitor", "Gardener", "Doctor", "Psychologist", 'BoldHost', 'CameraMan', "Warden", "Chief", "Foreman", 'Sniper', "Accountant", "Lawyer"}, needAdditionalAid, service_range, true)
	if h then
		print('found one')
...
function needAdditionalAid(e)
	...
	return (e.Damage < 1 and e.Damage > 0.1 and (e.Misbehavior == nil or e.Misbehavior == 'None') and sameRoom(e) and (e.Vel.x * e.Vel.x + e.Vel.y * e.Vel.y < 0.001))
end
