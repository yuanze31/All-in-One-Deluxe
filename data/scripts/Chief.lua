...
function Init()
    ...
    Interface.AddComponent(this, "fc__toggle_corruption", 'Button', "fc__toggle_corruption",
        tostring(this.corruption_enabled), 'B')
    Interface.AddComponent(this, "fc__change_corruption_difficulty", 'Button', "fc__change_corruption_difficulty",
        this.corruption_difficulty, 'N')
...
function fc__change_corruption_difficultyClicked()
    ...
    Interface.SetCaption(this, "fc__change_corruption_difficulty", "fc__change_corruption_difficulty",
        this.corruption_difficulty, 'N')
end
...
function corruption()
    if this.corruption_enabled then
        ...
        for g, d in pairs(gs) do
            if g.Damage >= 0.7 or (g.last_investigation and g.last_investigation + 60 * 4 > World.TimeIndex) then
            else
...
function _callG(g)
    if g.Damage >= 0.7 or g.JobId > 0 or g.Carrying.u > 0 then
        return
    end
...
function updateDiciplineBonus(b)
    b.Tooltip = {'fc__tt__bonus', b.level, 'L', b.extortion_count, 'X', b.beat_count, 'B', b.fight_count, 'F', b.money,
                 'M'}
end
...
function updateTooltip()
    ...
    if this.corruption_enabled then
        ...
        this.Tooltip = {'chief__tt__fc_on', sizeOf(corrupt_guards), 'N', overall_level, 'L', this.investigation_count,
                        'I', this.found_guard, 'G', cr, 'R'}
    else
